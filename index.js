...
export default e => {
  ...
  class TrailMesh extends THREE.Mesh {
    ...
    update(enabled, matrixWorld) {
      ...
      //console.log(app.children[0].physicsObjects[0] === subApp.physicsObjects[0]);

      //physics.enableGeometry(subApp.physicsObjects[0]);
      //physics.enableGeometry(subApp.physicsObjects[0]);
      //subApp.physicsObjects[0];
  ...
  e.waitUntil((async () => {
    ...
    //collisionDebug = new THREE.Mesh(geometry, material);

    collisionObject = physics.addBoxGeometry(subApp.position, subApp.quaternion, new THREE.Vector3(.5, .5, .5), true);

    //app.add(collisionDebug);

    //console.log(subApp, u2, m);

    

    //subAppPhysicsObject = physics.addGeometry(subApp.glb.clone());
    //subApp.add(subAppPhysicsObject);
  ...
  useFrame(() => {
    ...
    if(collisionObject) {
      collisionObject.position.copy(app.position);
      collisionObject.quaternion.copy(app.quaternion);
      collisionObject.updateMatrixWorld();
      physics.setTransform(collisionObject);

      // collisionDebug.position.copy(collisionObject.position);
      // collisionDebug.quaternion.copy(collisionObject.quaternion);
      // collisionDebug.updateMatrixWorld();

      // console.log(collisionObject.position);
    }
